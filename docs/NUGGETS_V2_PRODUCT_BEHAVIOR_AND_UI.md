...
- **Home mobile card opt-out:** `/?view=grid` switches to standard cards; segmented toggle in the feed filter rail (neutral track, yellow active segment matching filter chips).
- **Skim row content:** Title (2 lines), 1-line plain `card_preview`, published date (`Jun 9, 2026`) + primary tag (domain tag on Pulse stream), 96×54 (16:9) static thumb — `object-cover` for YouTube, `object-contain` for static images — no hero video, no blockquote HTML, no per-row bookmark/share.
