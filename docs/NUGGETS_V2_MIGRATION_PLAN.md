...
| `created_at` / `createdAt` Mongo (if any) | `created_at` timestamptz | **createdAt note — confirmed:** no `createdAt` / `created_at` field exists on any document in the `articles` collection. ETL derives `created_at` from ObjectId timestamp using `new Date(parseInt(doc._id.toString().slice(0, 8), 16) * 1000)`. No further field check needed in PR-15. |
| `status`, `visibility`, publishability | **`articles.status`** (`'draft'` \| `'published'`) — **single source of truth** (`BLUEPRINT` §2.a). `visibility` and `approval_*` **omitted** in Postgres. | **`is_published` does NOT exist in v2 schema** (§2.a). Map Mongo published/draft semantics directly into `status`. **status / isPublished note:** status field has ~2% explicit presence. The dominant boolean is `isPublished`. ETL mapping: `isPublished === true` → `status='published'`; `isPublished === false`, `null`, or field absent → `status='draft'`. If both `status` and `isPublished` exist on a document, `status` wins. |
...
| (generated) | **`articles.slug`** | Generated at insert via shared util `slugify(title) + '-' + substr(replace(id::text,'-',''),1,6)` — **same util as admin** (§3.1, `BLUEPRINT` §2.a). UNIQUE — never collides because of UUID-derived suffix. **slug note:** `slug` is absent on all legacy documents (0% presence). ETL generates every slug fresh using `generateArticleSlug(title, id)` from `scripts/shared/slug.ts`. No legacy slug is preserved. |
| **`contentStream`**: `standard` \| `pulse` \| **`both`** | Postgres **`standard` \| `pulse` only** — **one row per Mongo doc** | **`both`** → **`standard`** (founder: **`both`** unlikely). **`pulse`** / **`standard`** unchanged. **content_stream note:** `content_stream` has ~24% explicit presence. Remaining documents have no stream field. ETL default: missing or null → `content_stream='standard'`. The value `'both'` (rare legacy) → `content_stream='standard'`. |
