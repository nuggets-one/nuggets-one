...
| **Search API contract** | **`GET` only** for **`/api/search/suggest`** and **`/api/search`**. Global-by-default when **`NEXT_PUBLIC_SEARCH_GLOBAL`** is enabled: both endpoints search **across all streams/scopes** (`stream=all`, no scope), and **stream/scope/tags are optional narrowing facets** — committing a search clears the active section facets. When the flag is **off**, legacy stream-scoped behavior applies: **`stream`** query param **required** on both (default **`standard`** at handler boundary), results **stream-scoped**, and **live suggestions** send the **same `stream`** as Home. |
