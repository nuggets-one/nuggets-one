...
**Resolved:** Canonical bookmarks route is **`/bookmarks`**; **no** legacy path redirects required at launch (greenfield). **Aligned with `BLUEPRINT` §2.a:** the Next.js app lives at the **repository root** (`app/`, `components/`, `lib/`, root `package.json`). This repo is **greenfield-only** — no parallel `web/` subfolder and no legacy Vite/Express tree to preserve. (**§10 checklist.**)
...
| **Exact scope** | `create-next-app`-equivalent tree at **repo root** (`app/layout.tsx`, `app/page.tsx` placeholder, `package.json`, `tsconfig.json`, `next.config.ts`, `.env.example`), **`README`** snippet "how to run". **`package.json`** is **greenfield** — include only the v2 dependency set. **Repo layout (frozen — `BLUEPRINT` §2.a):** Next.js at repository root — no `web/` subfolder; monorepo / `apps/web` nesting out of scope unless the blueprint is explicitly revised. |
| **Files touched** | **`app/`**, **`components/`**, root config (`package.json`, `tsconfig.json`, `next.config.ts`, Tailwind, ESLint) — new tree for this greenfield repo. |
| **Dependencies (v2 PMF runtime)** | Node 20+. **Required at PR-01:** `next`, `react`, `react-dom`, `typescript`, `tailwindcss`, `@tailwindcss/typography`, `lucide-react`, `tailwind-merge`, `clsx`, `zod`. **Added later:** `@supabase/ssr` + `@supabase/supabase-js` (PR-03), `next-themes` (PR-03b), `nuqs` (PR-06), `react-markdown` + `remark-gfm` (PR-08), `@vercel/analytics` (PR-17), `react-hook-form` (PR-14, **admin only**). **Forbidden in `package.json`** (`BLUEPRINT` §19 + §2.a): `express`, `helmet`, `cors`, `compression`, `morgan`, `multer`, `cookie-parser`, `express-rate-limit`, `bcryptjs`, `jsonwebtoken`, `bullmq`, `redis`, `rate-limit-redis`, `mongoose`, `web-push`, `react-router-dom`, `@tanstack/react-query`, `@tanstack/react-virtual`, `@google/genai`, `open-graph-scraper`, `resend`, `papaparse`, `xlsx`, `probe-image-size`, `moment`/`date-fns`/`dayjs`, any client state library (Redux/Zustand/Jotai/etc.), **`framer-motion`**, **`react-spring`**, **`auto-animate`**, **`slugify`** (npm package — use `scripts/shared/slug.ts`), **`@vercel/og`**, **`react-modal`**, **`@headlessui/react`** (in public chunks), **`react-youtube`**, **`react-player`**, **`@sentry/react`** / **`@sentry/nextjs`** (Vercel logs only PMF). CI grep gate at PR-01 close. |
| **CI grep gates** (PR-01 close — fail build on any match) | `! grep -rE "\bis_published\b" app/ components/ lib/ scripts/ supabase/ db/`<br>`! grep -rE "from ['\"]framer-motion['\"]" app/ components/ lib/`<br>`! grep -rE "rehype-raw" app/ components/ lib/`<br>`! grep -rE "service_role\|SUPABASE_SERVICE_ROLE_KEY" components/ app/ --include='*.tsx' --include='*.ts' \| grep -v "import 'server-only'"` |
...
| **Rollback** | Revert scaffold commit / restore tree. |
...
| **Files touched** | `app/page.tsx`, `components/feed/**`, `components/article/ArticleCard*.tsx`, `components/chrome/Header.tsx`, `components/chrome/StreamTabs.tsx`, `components/chrome/TagChipRail.tsx`, `components/chrome/ActiveFiltersBar.tsx`. |
...
- **Next tree** lives at **repository root** (frozen — `BLUEPRINT` §2.a): `app/`, `components/`, `lib/`, root `package.json`, `tsconfig.json`, `next.config.ts`, `tailwind.config.ts`, ESLint — greenfield-only; companion docs may still say `web/` as an alias for these paths.
