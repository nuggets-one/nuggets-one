...
export function MobileBottomNav() {
  ...
  const cls = (active: boolean) =>
    `flex min-h-[44px] min-w-0 flex-1 flex-col items-center justify-center px-1.5 pb-2 pt-[10px] text-center text-xs font-semibold leading-tight tracking-tight transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus/60 ${
      active ? 'border-t-[3px] border-accent bg-surface-raised text-primary' : 'border-t-[3px] border-transparent text-muted hover:text-primary'
    }`

  return (
    <nav
      aria-label="Mobile"
      className="fixed bottom-0 left-0 right-0 z-40 lg:hidden border-t border-border bg-bg/95 pb-[env(safe-area-inset-bottom)] backdrop-blur-sm supports-[backdrop-filter]:bg-bg/80"
    >
